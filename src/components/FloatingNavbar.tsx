...
export function FloatingNavbar() {
  ...
  return (
    <div className={cn(
      ...
      {/* Floating Container — glassmorphism crystal at 80% opacity */}
      <div className="flex flex-col h-full m-4 rounded-2xl transition-all duration-300 bg-[#09291D]/80 backdrop-blur-[20px] border border-white/10">
        ...
        <div className="flex flex-col items-center pt-4 pb-3 border-b border-white/5">
          <div
            className="flex h-12 w-12 items-center justify-center rounded-2xl border border-[#C8A97A]/30 bg-[#C8A97A]/10 transition-transform duration-300 hover:scale-105 shadow-lg shadow-black/30 overflow-hidden"
            aria-label="EcoTracer"
          >
            <img
        ...
        <div className={cn(
          ...
          {/* Wallet status — only shown when expanded */}
          {isExpanded && (
            <div className="mt-2 w-full px-1 opacity-0 animate-fade-up" style={{ animationFillMode: 'forwards' }}>
              {walletAddress ? (
                ...
                <button
                  ...
                  className="flex w-full items-center gap-1.5 rounded-lg border border-dashed border-white/12 px-2 py-1.5 text-[10px] text-white/40 transition-all hover:border-[#C8A97A]/40 hover:text-[#C8A97A] disabled:opacity-50"
                >
                  {isConnecting ? <Loader2 className="h-3 w-3 animate-spin" /> : <Wallet className="h-3 w-3" />}
                  <span>{isConnecting ? 'Conectando...' : 'Freighter'}</span>
                </button>
              )}
            </div>
          )}
        ...
        <nav className={cn(
          ...
          <NavItem to="/"       icon={<LayoutDashboard className="h-5 w-5" />} label="Dashboard" isExpanded={isExpanded} end />
          <NavItem to="/lotes"  icon={<Package className="h-5 w-5" />}         label="Lotes"     isExpanded={isExpanded} />
          <NavItem to="/wallet" icon={<Wallet className="h-5 w-5" />}          label="Wallet"    isExpanded={isExpanded} />
        </nav>

        {/* Footer — Logout with confirmation popover */}
        <div className={cn(
